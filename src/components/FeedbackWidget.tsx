...
function CropOverlay({ preview, onConfirm, onCancel }: CropOverlayProps) {
  ...
  const onMouseDown = (e: React.MouseEvent) => {
    ...
    setStart(pt); setSel(null); setDragging(true);
  };
    ...
    <div className="fixed inset-0 z-[100] flex flex-col bg-black/90">
      <div className="flex items-center justify-between shrink-0 px-4 py-3 bg-slate-900 border-b border-slate-700">
        <div>
          ...
          <p className="text-[11px] text-slate-400">Click and drag to select a region, then click Crop</p>
        </div>
        <div className="flex items-center gap-2">
          {sel && sel.w > 4 && sel.h > 4 && (
            <button type="button" onClick={() => cropAndConfirm(sel)}
          ...
          <button type="button" onClick={() => cropAndConfirm(null)}
      ...
      <div className="relative flex flex-1 items-center justify-center overflow-hidden p-6 select-none">
        <div className="relative" onMouseDown={onMouseDown} style={{ cursor: 'crosshair' }}>
          <img ref={imgRef} src={preview} alt="captured screenshot"
            className="max-w-full max-h-[calc(100vh-120px)] rounded shadow-2xl block" draggable={false} />
          {sel && sel.w > 2 && sel.h > 2 && (
            <svg className="absolute inset-0 pointer-events-none" width="100%" height="100%">
              ...
              <rect x={sel.x} y={sel.y} width={sel.w} height={sel.h} fill="none" stroke="#3b82f6" strokeWidth="2" strokeDasharray="6 3" />
              {([[sel.x, sel.y],[sel.x+sel.w, sel.y],[sel.x, sel.y+sel.h],[sel.x+sel.w, sel.y+sel.h]] as [number,number][]).map(([cx,cy], i) => (
                <rect key={i} x={cx-4} y={cy-4} width={8} height={8} fill="white" stroke="#3b82f6" strokeWidth="1.5" rx="1" />
              ))}
              <text x={sel.x + sel.w / 2} y={sel.y - 6} textAnchor="middle" fontSize="11" fill="white" style={{ userSelect: 'none' }}>
...
async function captureScreen(): Promise<string> {
  ...
  return new Promise((resolve, reject) => {
    ...
    track?.addEventListener('ended', () => { cleanup(); reject(new DOMException('Screen share ended', 'AbortError')); }, { once: true });
...
export function FeedbackWidget({
  ...
  useEffect(() => {
    ...
    setResult(''); setRating(null); setScreenshots([]); setError(null);
    setStep('goal');
    onTaskTriggerConsumed?.();
  ...
  return (
    <>
      {cropPreview && (
        <CropOverlay
          preview={cropPreview}
          onConfirm={(shot) => { setScreenshots(prev => [...prev, shot]); setCropPreview(null); }}
          ...
          <div className={cn(
            ...
            <div className="px-4 py-4">
              ...
              {step === 'screenshot' && (
                <div className="flex flex-col gap-3">
                  ...
                  <p className="text-[11px] text-slate-500 leading-snug">Capture your screen — you can crop to a specific area after.</p>
                  ...
                  <div className="flex gap-2 pt-1">
                    ...
                    <button type="button" disabled={loading} onClick={submit}
                      ...
                      {loading
                        ...
                        : <><Send className="size-3.5" />{screenshots.length > 0 ? `Send (${screenshots.length})` : 'Send without'}</>}
                    </button>
                  </div>
                </div>
              )}
            ...
            {step !== 'done' && (
              <div className="border-t border-slate-100 bg-slate-50 px-4 py-2">
                {activeTask
                  ? <p className="text-[10px] text-slate-400 truncate">📋 {activeTask.taskTitle} · 📍 {view}</p>
